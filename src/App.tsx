...
const PresentialCourseDetail = ({ course, onUpdate, onBack, teachers, files }: PresentialDetailProps) => {
    ...
    const [fileSelectModal, setFileSelectModal] = useState<{
        open: boolean;
        disciplineId: string;
        topicId: string;
        moduleId: string;
        type: 'MATERIAL' | 'QUESTION';
    }>({ open: false, disciplineId: '', topicId: '', moduleId: '', type: 'MATERIAL' });
    ...
    // ... (Schedule Generation Logic kept same as before, simplified for brevity in thought but must be present in code)
    ...
    const openFileSelection = (dId: string, tId: string, mId: string, type: 'MATERIAL' | 'QUESTION') => {
        setFileSelectModal({ open: true, disciplineId: dId, topicId: tId, moduleId: mId, type });
    };

    const handleFileSelected = (file: FileNode) => {
        if (file.type !== 'FILE') return;
        const { disciplineId, topicId, moduleId, type } = fileSelectModal;
        
        setLocalCourse(prev => ({
            ...prev,
            disciplines: prev.disciplines.map(d => d.id === disciplineId ? {
                ...d,
                ...
                setLocalCourse(prev => ({
                    ...
                    disciplines: prev.disciplines.map(d => d.id === newItemModal.dId ? {
                        ...
                        topics: d.topics.map(t => t.id === newItemModal.tId ? {
                            ...t,
                            modules: t.modules.map(m => m.id === newItemModal.tId ? {...t, modules: [...t.modules, newM]} : t)
                        } : t)
                    } : d)
                }));
            }
    ...
    return (
        <div className="h-full flex flex-col bg-insanus-black">
             ...
             <div className="flex-1 overflow-y-auto p-6 relative">
                ...
                {activeTab === 'EDITAL' && (
                    <div className="space-y-6">
                        ...
                        {localCourse.disciplines.map(d => {
                            // Calculation per discipline
                            let dClasses = 0;
                            d.topics.forEach(t => {
                                t.modules.forEach(m => {
                                    if (m.isSelectedForPresential) {
                                        dClasses += (m.requiredClasses || 1);
                                    }
                                });
                            });
                            const dMeetings = Math.ceil(dClasses / cfg.classesPerMeeting);
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1 ml-7">
                                                    <span className="text-[10px] font-bold bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded border border-blue-500/20 flex items-center gap-1">
                                                        <Clock size={10} /> {dClasses} Aulas
                                                    </span>
                                                    <span className="text-[10px] font-bold bg-purple-500/10 text-purple-400 px-2 py-0.5 rounded border border-purple-500/20 flex items-center gap-1">
                                                        <CalendarDays size={10} /> {dMeetings} Encontros
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                ...
                {fileSelectModal.open && (
                    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
                        <div className="bg-insanus-panel border border-gray-700 rounded-xl w-full max-w-4xl h-[80vh] flex flex-col shadow-2xl">
                            <div className="p-4 border-b border-gray-800 bg-insanus-dark flex justify-between items-center">
                                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                                    {fileSelectModal.type === 'MATERIAL' ? <FileText className="text-green-500"/> : <FileQuestion className="text-yellow-500"/>}
                                    Selecionar {fileSelectModal.type === 'MATERIAL' ? 'Material PDF' : 'Lista de Questões'}
                                </h3>
                                <button onClick={() => setFileSelectModal(prev => ({...prev, open: false}))} className="text-gray-400 hover:text-white"><X /></button>
                            </div>
...
// 4. Course Detail (ONLINE)
const CourseDetail = ({ course, onUpdate, onBack }: { course: Course, onUpdate: (c: Course) => Promise<void>, onBack: () => void }) => {
    // Simple placeholder implementation for online course management
    return (
        <div className="h-full flex flex-col p-6 bg-insanus-black">
             <div className="flex items-center gap-4 mb-6">
                <button onClick={onBack} className="p-2 hover:bg-gray-800 rounded-full text-white"><ArrowLeft /></button>
                <h2 className="text-2xl font-bold text-white">{course.name} <span className="text-sm font-normal text-gray-400 ml-2">(Online)</span></h2>
            </div>
            <div className="flex-1 flex items-center justify-center text-gray-500 flex-col gap-4 border border-gray-800 rounded-xl bg-insanus-panel/20">
                <MonitorPlay size={48} />
                <p>Gestão de Turmas Online em Desenvolvimento</p>
                <p className="text-sm">Utilize a aba de Arquivos para gerenciar materiais por enquanto.</p>
            </div>
        </div>
    );
};
...
const App = () => {
  ...
  const handleRemoveSubject = (sub: string) => {
      setTeacherFormData(prev => ({
          ...
          primarySubject: prev.primarySubject === sub ? (prev.subjects.find(s => s !== sub) || '') : prev.primarySubject
      }));
  };

  const handleOpenEditSubject = (sub: string) => {
      setInputDialog({
          open: true,
          ...
          onConfirm: (newValue) => {
              ...
              setTeacherFormData(prev => ({
                  ...prev,
                  subjects: prev.subjects.map(s => s === sub ? newValue : s),
                  primarySubject: prev.primarySubject === sub ? newValue : prev.primarySubject
              }));
    ...
    <div className="flex bg-insanus-black min-h-screen text-gray-100 font-sans">
      ...
      <main className="flex-1 overflow-hidden relative">
        ...
        {isScheduleModalOpen && (
                  ...
                  <div className="p-6 overflow-y-auto space-y-8">
                    ...
                    <div className="flex justify-end pt-4 border-t border-gray-800">
                        <button 
                            ...
                            className="bg-insanus-red hover:bg-red-700 text-white font-bold py-3 px-8 rounded-lg transition-colors flex items-center gap-2"
                        >
                            {creatingCourse ? <Loader2 className="animate-spin" /> : <Check />} 
                            Criar Projeto
                        </button>
                    </div>
                  </div>
         ...
         {isTeacherModalOpen && (
             <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                 <div className="bg-insanus-panel border border-gray-700 rounded-xl w-full max-w-2xl p-6 shadow-2xl flex flex-col max-h-[90vh]">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold text-white">{editingTeacherId ? 'Editar Professor' : 'Novo Professor'}</h3>
                        <button onClick={() => setIsTeacherModalOpen(false)} className="text-gray-400 hover:text-white"><X /></button>
                    </div>
                    <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                        <div className="flex items-center gap-6 mb-6">
                            <div className="relative group cursor-pointer">
                                <div className="w-24 h-24 rounded-full bg-gray-800 border-2 border-dashed border-gray-600 flex items-center justify-center overflow-hidden">
                                    {teacherFormData.avatarUrl ? <img src={teacherFormData.avatarUrl} className="w-full h-full object-cover"/> : <Camera className="text-gray-500"/>}
                                </div>
                                <div className="absolute inset-0 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <span className="text-xs text-white font-bold">Alterar</span>
                                </div>
                                <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={handleAvatarSelect} />
                            </div>
                            <div className="flex-1">
                                <label className="text-xs text-gray-400 uppercase font-bold block mb-1">Nome Completo</label>
                                <input type="text" className="w-full bg-black border border-gray-700 rounded p-2 text-white mb-2" value={teacherFormData.name} onChange={e => setTeacherFormData({...teacherFormData, name: e.target.value})} />
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs text-gray-400 uppercase font-bold block mb-1">Email</label>
                                        <input type="email" className="w-full bg-black border border-gray-700 rounded p-2 text-white" value={teacherFormData.email} onChange={e => setTeacherFormData({...teacherFormData, email: e.target.value})} />
                                    </div>
                                    <div>
                                        <label className="text-xs text-gray-400 uppercase font-bold block mb-1">WhatsApp</label>
                                        <input type="text" className="w-full bg-black border border-gray-700 rounded p-2 text-white" value={teacherFormData.whatsapp} onChange={e => setTeacherFormData({...teacherFormData, whatsapp: e.target.value})} />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select className="w-full bg-black border border-gray-700 rounded p-2 text-white" value={teacherFormData.segment} onChange={e => setTeacherFormData({...teacherFormData, segment: e.target.value as any})}>
                            ...
                            <div>
                                ...
                                <input type="text" className="w-full bg-black border border-gray-700 rounded p-2 text-white" value={teacherFormData.primarySubject} onChange={e => setTeacherFormData({...teacherFormData, primarySubject: e.target.value})} />
                            </div>
                        </div>

                        <div>
                            <label className="text-xs text-gray-400 uppercase font-bold block mb-1">Disciplinas (Lista)</label>
                            <div className="flex gap-2 mb-2">
                                <input type="text" className="flex-1 bg-black border border-gray-700 rounded p-2 text-white" placeholder="Nova disciplina..." value={newSubject} onChange={e => setNewSubject(e.target.value)} onKeyDown={e => e.key === 'Enter' && addSubject()} />
                                <button onClick={addSubject} className="bg-gray-700 hover:bg-gray-600 text-white px-4 rounded"><Plus/></button>
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {teacherFormData.subjects.map(sub => (
                                    <span key={sub} className="bg-gray-800 text-gray-300 text-xs px-2 py-1 rounded border border-gray-700 flex items-center gap-1 group">
                                        {sub}
                                        <button onClick={() => handleOpenEditSubject(sub)} className="hidden group-hover:block text-blue-400 hover:text-blue-300 ml-1"><Edit size={10}/></button>
                                        <button onClick={() => handleRemoveSubject(sub)} className="hover:text-red-500 ml-1"><X size={12}/></button>
                                    </span>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="mt-6 flex justify-end">
                        <button onClick={handleSaveTeacher} className="bg-insanus-red hover:bg-red-700 text-white font-bold py-2 px-6 rounded">Salvar Professor</button>
                    </div>
                 </div>
             </div>
         )}
